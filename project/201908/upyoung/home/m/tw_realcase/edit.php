...
//===========================================================================
	...
	if( isset($_POST['vif']) && $_POST['vif']=='1' ){  //新增、修改
		$message_str='';
		up_file_photo($sys_updata, $mfilename, 'mr',
		...
		if( $$mpk==0 ){
			$sql='insert into '.$mtable.' ('.$mpk.', menable, '.$mfname.', msort, mimage, date_create, date_modify) VALUES ( null, \''.$menable.'\', \''.$$mfname.'\', \''.$msort.'\', \''.$upfilearray[0][1].'\', \''.$now_date.'\', \''.$now_date.'\')';
			$save_note='新增：'.$$mfname;
		}else{
			$sql='update '.$mtable.' set menable=\''.$menable.'\', '.$mfname.'=\''.$$mfname.'\', '.$msort_str.'mimage=\''.$upfilearray[0][1].'\', date_modify=\''.$now_date.'\' where '.$mpk.'=\''.$$mpk.'\'';
			$save_note='修改：'.$$mfname;
		}
		$result->SQLrequest($sql);
